...
def test_edges_keep_grid_boundary_roads_but_exclude_stub_connectors() -> None:
    ...
    assert 'N0_0_to_N0_1' in edge_ids
    assert 'N0_1_to_N0_0' in edge_ids
    assert 'N3_2_to_N3_3' in edge_ids
    assert 'N3_3_to_N3_2' in edge_ids
    assert 'N1_0_to_N2_0' in edge_ids
    assert 'N2_0_to_N1_0' in edge_ids
    assert 'N1_3_to_N2_3' in edge_ids
    assert 'N2_3_to_N1_3' in edge_ids
    assert 'S_top_1_to_N0_1' in edge_ids
    assert 'N0_1_to_N1_1' in edge_ids
    assert 'S_top_0_to_S_top_1' not in edge_ids
    assert 'S_top_1_to_S_top_0' not in edge_ids
    assert 'S_left_1_to_S_left_2' not in edge_ids
    assert 'S_left_2_to_S_left_1' not in edge_ids
...
def test_connections_only_reference_existing_source_lanes_with_stubs() -> None:
    ...
    connections = build_connection_specs(nodes + stub_nodes, edges)

    assert all(connection.from_lane < lanes_by_edge[connection.from_edge] for connection in connections)
...
def test_boundary_stubs_add_external_spawn_nodes_on_every_boundary_approach() -> None:
    ...
    assert len(stub_nodes) == 12
    assert {stub_node.node_type for stub_node in stub_nodes} == {None}
    assert {stub_node.node_id for stub_node in stub_nodes} >= {
        'S_top_0',
        'S_bottom_2',
        'S_left_1',
        'S_right_1',
...
def test_default_route_flows_use_external_stubs_on_all_boundary_sides() -> None:
    ...
    assert len(flows) == 12
    assert source_nodes == {stub_node.node_id for stub_node in stub_nodes}
    assert destination_nodes == {stub_node.node_id for stub_node in stub_nodes}
