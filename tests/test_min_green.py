...
def test_min_green_reports_when_switching_is_legal() -> None:
    controller = MinGreenController(min_green_steps=2)

    assert controller.can_switch('J0') is True
    controller.filter_targets({'J0': 'Grr'})
    assert controller.current_target('J0') == 'Grr'
    assert controller.can_switch('J0') is False
    controller.filter_targets({'J0': 'Grr'})
    assert controller.can_switch('J0') is True
