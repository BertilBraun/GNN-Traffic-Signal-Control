...
from scripts.build_network import _is_netconvert_internal_abort, _netconvert_osm_import_command, _strip_plain_edge_types
...
def test_netconvert_osm_import_command_can_disable_sumo_joining(tmp_path: Path) -> None:
    osm_path = tmp_path / 'city.osm'
    net_path = tmp_path / 'city.net.xml'

    joined_command = _netconvert_osm_import_command(
        osm_path=osm_path,
        net_path=net_path,
        join_dist=40.0,
    ...
    assert '--junctions.join' in joined_command
    assert '--tls.join' in joined_command
