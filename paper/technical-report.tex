...
The policies transferred within the synthetic grid family, while changes in signal coverage exposed a clear distribution shift.
...
Grid results separate three policy-training seeds from six previously unused traffic seeds.
...
\begin{itemize}
  \item \textbf{RQ1:} three mixed-grid training seeds, trained on five square or rectangular geometries whose longest axis is five; final evaluation on ten geometries at demands \(0.6,0.7,0.8\), including \(6\times6\), with six previously unused traffic seeds.
  \item \textbf{RQ2:} a full-coverage-trained \(6\times6\) policy evaluated after removing signals, plus one separate training seed exposed to five \(50\%\)-coverage \(4\times4\) layouts and evaluated on previously unused \(25\%\)--\(100\%\) layouts.
  \item \textbf{RQ3:} one city policy-training seed with rollouts from Karlsruhe, Mannheim, Heidelberg, and Freiburg; Stuttgart had no rollout jobs but was visible in periodic evaluation. The fixed iteration-60 instance was evaluated for \(1200\,\mathrm{s}\) at demand scale 1.0 on three traffic seeds in all five cities.
\end{itemize}
...
Execution on graphs ranging from 41 to 84 controlled junctions and phase-count ranges of 2--12 provides implementation validation, not the basis of the property.
...
In the separate \(4\times4\) study, a policy trained on \(50\%\)-coverage layouts was close to max pressure under sampled execution at previously unused \(25\%\) and \(50\%\) layouts, but its observed throughput and completion were lower at \(75\%\).
...
The aggregate comparison instead uses max pressure consistently: for the unweighted mean across the five city-level results, the sampled policy achieved \(3345.6\) vehicles/hour and \(64.93\%\) completion in this run, versus \(3036.8\) and \(59.37\%\) for max pressure.
...
\begin{table}[H]
  \centering
  \caption{Full-coverage \(6\times6\) result. Learned values average three training seeds and six previously unused traffic seeds; max pressure averages the six traffic seeds.}
  \begin{tabular}{@{}llrrr@{}}
    \toprule
...
Across the unweighted mean of the five city-level results, greedy execution achieved \(3182.6\) vehicles/hour, \(61.89\%\) completion, and \(0.980\,\mathrm{s/m}\) wait density, compared with \(3345.6\), \(64.93\%\), and \(0.613\,\mathrm{s/m}\) for sampled execution.

\paragraph{Coverage study.}
The \(50\%\)-coverage training study used five training layouts and three previously unused traffic seeds on each of five evaluation layouts at \(25\%\), \(50\%\), and \(75\%\) coverage.
