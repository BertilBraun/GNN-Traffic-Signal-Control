...
\begin{description}[leftmargin=4.2em,style=nextline]
  \item[RQ1 --- Synthetic transfer.] How does the trained policy behave on unseen grid sizes and aspect ratios produced by the same generator?
...
\begin{figure}[H]
  \centering
  \resizebox{\reportcontentwidth}{!}{\input{figures/intersection-concepts.tex}}
...
\begin{figure}[H]
  \centering
  \resizebox{\reportcontentwidth}{!}{\input{figures/representation.tex}}
...
\begin{figure}[H]
  \centering
  \resizebox{\reportcontentwidth}{!}{\input{figures/phase-interface.tex}}
...
The baselines therefore do not share the learned policy's decision frequency, and comparisons include this implementation difference.
...
\begin{figure}[H]
  \centering
  \includegraphics[width=\reportcontentwidth]{figures/evaluation-summary.pdf}
  \caption{Observed feasibility results. Left: sampled \(6\times6\) performance by demand. Right: iteration-60 city outcomes relative to each city's strongest non-learned throughput baseline.}
  \label{fig:results}
\end{figure}
\FloatBarrier
...
That separation provides variable graph and action dimensions by construction.
...
\begin{table}[H]
  \centering
  \caption{Full-coverage \(6 \times 6\) result. Learned values average three training seeds and six held-out traffic seeds; max pressure averages the six traffic seeds.}
  \begin{tabular}{@{}llrrr@{}}
    \toprule
...
\begin{table}[H]
  \centering
  \caption{Iteration-60 city means over three traffic seeds. The baseline has the highest throughput among non-learned policies in that city. Wait density is in seconds per metre.}
  \begin{tabular}{@{}llrrr@{}}
    \toprule
