...
function movementPosition(movement) {{
  ...
  const sameInput = movementsByTls
    ...
    .sort((left, right) => {{
      const leftOutput = lanePosition(laneById.get(left.output_lane_group_id));
      const rightOutput = lanePosition(laneById.get(right.output_lane_group_id));
      const leftProjection = (leftOutput.x - center.x) * -unitY + (leftOutput.y - center.y) * unitX;
      const rightProjection = (rightOutput.x - center.x) * -unitY + (rightOutput.y - center.y) * unitX;
      return leftProjection - rightProjection || left.movement_id - right.movement_id;
    }});
