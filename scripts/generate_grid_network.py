...
def build_edge_specs(
    ...
    for node in nodes:
        for d_row, d_col in ((0, 1), (1, 0)):
            ...
            if other is None:
                continue
            if _is_stub_node(node.node_id) and _is_stub_node(other.node_id):
                continue
...
def build_connection_specs(
    ...
    for current in nodes:
        ...
        for incoming in sorted(incoming_by_node.get(current.node_id, []), key=lambda edge: edge.edge_id):
            candidates = [edge for edge in outgoing if edge.to_node != incoming.from_node]
            if len(candidates) == 1:
                target = candidates[0]
...
def generate_grid(
    ...
    node_path = output_dir / 'grid.nod.xml'
    edge_path = output_dir / 'grid.edg.xml'
    connection_path = output_dir / 'grid.con.xml'
    network_path = output_dir / 'grid.net.xml'

    _write_nodes(node_path, all_nodes)
    ...
    if netconvert:
        _run_netconvert(node_path, edge_path, connection_path, network_path)
        _write_tll_from_net(network_path, output_dir / 'grid.tll.xml')
    _print_summary(all_nodes, edges, connections, network_path)
    return network_path
...
def _opposite_boundary_edge(
    ...
    if _is_stub_node(source.from_node):
        ...
        candidates = [edge for edge in sink_edges if edge.to_node == opposite_stub]
        if candidates:
            return sorted(candidates, key=lambda edge: edge.edge_id)[0]
        far_side = [edge for edge in sink_edges if _stub_side(edge.to_node) != _stub_side(source.from_node)]
    ...
    candidates = [edge for edge in sink_edges if _parse_node_id(edge.to_node) == mirror]
    if candidates:
        return sorted(candidates, key=lambda edge: edge.edge_id)[0]
    far_side = [
        edge
...
def _internal_grid_bounds(edges: list[EdgeSpec]) -> tuple[int, int]:
    positions = [
        _parse_node_id(node) for edge in edges for node in (edge.from_node, edge.to_node) if not _is_stub_node(node)
    ]
    return (
