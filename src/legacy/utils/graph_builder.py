...
class GraphBuilder:
    ...
    def __init__(self, net, junction_infos: dict) -> None:
        ...
        self._jid_to_idx: dict[str, int] = {jid: i for i, jid in enumerate(self.junction_ids)}
        ...
        self._edge_index: torch.Tensor = torch.tensor([src_list, dst_list], dtype=torch.long)  # (2, E)
        self._edge_attr: torch.Tensor = torch.tensor(ef_norm, dtype=torch.float32)  # (E, 3)
